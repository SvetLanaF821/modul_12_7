...
bool IsKPeriodic(const string& txt, unsigned int k) {
    ...
    string pat;//в подмассив размером К записываем первые К элементов
    for (unsigned int ik = 0; ik < k; ++ik) {
        pat += txt[ik];
    }

    //двигаемся по массиву txt, начиная с К-ого элемента и проверяя равенство
...
int main()
{
    ...
    while (true) {
        ...
        if (cin.fail()) {                 // Если никакого извлечения не произошло
            cin.clear();                  // то сбрасываем все текущие флаги состояния 
            cin.ignore(32767, '\n');      // очищаем поток от мусора 
            cout << "Oshibka. K - chiclo.\n";                  // просим пользователя ввести кратность еще раз 
            continue;
        }
        ...
        }                                 // пользователя ввести кратность еще раз

        if (K <= 0) {                     // убеждаемся, что K является положительным числом 
